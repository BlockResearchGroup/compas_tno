...
for thk in [0.50, 0.45, 0.40, 0.35, 0.30, 0.25]:

    folder = os.path.join('/Users/mricardo/compas_dev/me/compl_energy/pavillion/two_wall_open', form_type) + '/'
    ...
    for key in form.vertices_where({'is_fixed': True}):
        ...
        if abs(x - xf) < 0.01:
            ...
            base_plot.append(Point(x, y, 0 - 0.2))
        if abs(x - x0) < 0.01:
            dXbi = [-1, 0, 0]
            vectors_plot.append(Vector(dXbi[0], dXbi[2], 0.0))
            base_plot.append(Point(x, y, 0 - 0.2))
