...
def set_height_constraint(form, zmax = 1.0):
    ...
    for key in form.vertices():
        form.vertex_attribute(key, 'ub', value = zmax)
...
def set_cross_vault_heights(form, xy_span = [[0.0,10.0],[0.0,10.0]], ub_lb=False, weights = False, thk = None, tol = 0.00, set_heights = False, update_loads = False, t = 10.0, b = 1.0):
    ...
    for key in form.vertices():
        ...
        if weights:
            form.vertex_attribute(key,name='weight',value = form.vertex_attribute(key,'pz'))
        if set_heights:
            form.vertex_attribute(key, 'z', value = round(z,2))
        if form.vertex_attribute(key, 'is_fixed') == True:
            form.vertex_attribute(key, 'b', value = [b,b])
    ...
    if update_loads:
        ...
        for key in form.vertices():
            pz = factor * form.vertex_attribute(key, 'pz')
...
def set_cross_vault_heights_ub(form, xy_span = [[0.0,10.0],[0.0,10.0]], thk = 0.5, tol = 0.000, set_heights = False):
    ...
    for key in form.vertices():
        ...
        if yi <= y0 + (y1 - y0)/(x1 - x0) * (xi - x0) + tol and yi >= y1 - (y1 - y0)/(x1 - x0) * (xi - x0) + tol: #Q1
            ...
            print('Vertex {0} did not belong to any Q. (x,y) = ({1},{2})'.format(key,xi,yi))
            z = 0.0
        form.vertex_attribute(key, 'ub', value = z)
        if form.vertex_attribute(key, 'is_fixed'):
            form.attributes['tmax'] = z
        if set_heights:
            form.vertex_attribute(key,'z',value=z)
    ...
    for key in form.vertices():
        ...
        if ((yi) > y1 and ((xi) > x1 or (xi) < x0 )) or ((yi) < y0 and ((xi) > x1 or (xi) < x0)):
            ...
            if yi <= y0 + (y1 - y0)/(x1 - x0) * (xi - x0) + tol and yi >= y1 - (y1 - y0)/(x1 - x0) * (xi - x0) + tol: #Q1
                ...
                print('Vertex {0} did not belong to any Q. (x,y) = ({1},{2})'.format(key,xi,yi))
                z = 0.0
        form.vertex_attribute(key,'lb',value=z)
        if set_heights:
            form.vertex_attribute(key,'z',value=z)
...
def set_pavillion_vault_heights(form, xy_span = [[0.0,10.0],[0.0,10.0]], ub_lb = False, thk = 0.5, tol = 0.00, set_heights = False, b = 5.0, t = 2.0, update_loads = False, delete_boundary_edges = False):
    ...
    for key in form.vertices():
        ...
        if set_heights:
            form.vertex_attribute(key,'z',value=round(z,2))
        if form.vertex_attribute(key, 'is_fixed') == True:
            form.vertex_attribute(key, 'b', value = [b,b]) # Change to be [b,0] or [0,b]
...
def set_pointed_vault_heights(form, xy_span = [[0.0,10.0],[0.0,10.0]], hc=8.0, he=None, hm=None,  ub_lb=False, thk = None, tol = 0.00, set_heights = False):
    ...
    for key in form.vertices():
        ...
        if yi <= y0 + (y1 - y0)/(x1 - x0) * (xi - x0) + tol and yi >= y1 - (y1 - y0)/(x1 - x0) * (xi - x0) + tol: # Q1
            ...
            print('Vertex {0} did not belong to any Q. (x,y) = ({1},{2})'.format(key,xi,yi))
        form.vertex_attribute(key,'target',value=zi)

        if set_heights:
            form.vertex_attribute(key,'z',value=round(zi,2))
...
def circular_heights(form , x0 = None, xf = None, thk = 0.5, t = 20, density = 20, ly = 1.0, overwrite_weight=False):
    ...
    for key in form.vertices():
        ...
        if zi2 < 0:
            ...
            zi = math.sqrt(zi2)
        if form.vertex_attribute(key,'is_fixed') == True:
            form.vertex_attribute(key,'lb',value=-t)
            form.vertex_attribute(key,'ub',value=ze)
        else:
            form.vertex_attribute(key,'lb',value=zi)
            form.vertex_attribute(key,'ub',value=ze)
        ...
        if form.vertex_attribute(key, 'is_fixed') is True:
            pi = weight_unit / 2
            form.vertex_attribute(key, 'pz', pi)
            pzt += pi
        else:
            pi = weight_unit
            form.vertex_attribute(key, 'pz', pi)
