...
def save_geometry_at_iterations(form: FormDiagram, optimiser: Optimiser, force=False):
    ...
    for i in range(iterations):
        ...
        problem = xyz_from_xopt(xopt_i, problem)
        Xform_i = problem.X.tolist()
        Xform[str(i)] = Xform_i

        if force:
            j = 0
            for key in form.vertices():
                form.vertex_attributes(key, "xyz", problem.X[j].tolist())
                j += 1
            k = 0
